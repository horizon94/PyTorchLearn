...
class LSTMClassifier(nn.Module):
    ...
    def __init__(self, args, embedding_loader = None):
        ...
        self.hidden2class = nn.Linear(args.hidden_dim, args.class_num)
        self.hidden2article = nn.Linear(args.hidden_dim, args.articles_num)
    ...
    def forward(self, sentence):
        ...
        # print(h_n[0][0])
        # print("*"*50)
        scores_class  = self.hidden2class(h_n[-1])
        scores_article = self.hidden2article(h_n[-1])
        return scores_class, scores_article
